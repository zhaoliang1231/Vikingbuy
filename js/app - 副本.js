...
//			},
	...
	$.imageUpload = function(file){
		...
		var task = plus.uploader.createUpload( "http://192.168.1.233/vikingbuy/api/api.php", {
			method: "POST",
			blocksize: 204800,
			priority: 100
		},
		function(t, status){
			...
			if(status == 200){
				...
				uploads.files.push(result.result);
				console.log(uploads.files)
				mui.fire(plus.webview.currentWebview(), "uploadRefresh");
			}else{
				mui.toast("上传失败~");
			}
