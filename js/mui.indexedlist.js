...
(function($, window, document) {
	...
	var IndexedList = $.IndexedList = $.Class.extend({
		...
		findElements: function() {
			...
			self.el.news = self.box.querySelector(classSelector('indexed-new'));
		...
		caleLayout: function() {
			var self = this,withoutSearchHeight,barItemHeight;
			if(self.box.querySelector(classSelector('indexed-new'))){
				withoutSearchHeight = (self.box.offsetHeight - self.el.search.offsetHeight - self.el.news.offsetHeight -30) + 'px';
				self.el.bar.style.height = withoutSearchHeight;
				self.el.inner.style.height = withoutSearchHeight;
				barItemHeight = ((self.el.bar.offsetHeight -4) / self.el.barItems.length) + 'px';
				self.el.barItems.forEach(function(item) {
					item.style.height = barItemHeight;
					item.style.lineHeight = barItemHeight;
				});
			}else{
				withoutSearchHeight = (self.box.offsetHeight - self.el.search.offsetHeight) + 'px';
				self.el.bar.style.height = withoutSearchHeight;
				self.el.inner.style.height = withoutSearchHeight;
				barItemHeight = ((self.el.bar.offsetHeight -4) / self.el.barItems.length)-1 + 'px';
				self.el.barItems.forEach(function(item) {
					item.style.height = barItemHeight;
					item.style.lineHeight = barItemHeight;
				});
